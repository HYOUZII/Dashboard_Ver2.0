    ...
    container.innerHTML = `
        ...
        <div class="dashboard-grid">
            <div class="chart-card">
                ...
                <div id="gantt-chart-container">로딩 중...</div>
            </div>
            ...
            <div class="chart-card">
                ...
                <div id="interrupt-analysis">로딩 중...</div>
            </div>
            ...
            <div class="chart-card">
                ...
                <div id="productivity-dashboard">로딩 중...</div>
            </div>
        </div>
    `;
...
async function loadGanttChart(container) {
    ...
    try {
        ...
        if (!projects || projects.length === 0) {
            chartContainer.innerHTML = '<p>프로젝트가 없습니다.</p>';
            return;
        }
        
        let html = '<div class="gantt-chart">';
        
        const stageCounts = { '기획': 0, '설계': 0, '개발': 0, '테스트': 0, '양산': 0 };
        ...
        Object.entries(stageCounts).forEach(([stage, count]) => {
            html += `
                <div class="stat-box">
                    <div style="font-size: 14px; opacity: 0.9;">${stage}</div>
                    <div style="font-size: 32px; font-weight: bold; margin: 10px 0;">${count}</div>
                    <div style="font-size: 14px; opacity: 0.9;">개</div>
                </div>
            `;
        });
        html += '</div>';
        ...
        console.error('간트 차트 오류:', error);
        chartContainer.innerHTML = '<div style="color: red;">데이터 로딩 실패</div>';
    }
}
...
async function loadInterruptAnalysis(container) {
    ...
    try {
        ...
        const isHigh = totalHours > 50;
        
        let html = '<div class="interrupt-grid"><div><h4>부서별 인터럽트 시간</h4><div class="bar-chart">';
        ...
        html += `</div></div><div>
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 15px;">
                ...
                <button onclick="showDisruptionHelp()" style="background: #17a2b8; color: white; border: none; border-radius: 50%; width: 24px; height: 24px; cursor: pointer; font-size: 14px; font-weight: bold;">?</button>
            </div>
            <div style="background: linear-gradient(135deg, ${isHigh ? '#f093fb, #f5576c' : '#4facfe, #00f2fe'}); padding: 40px 20px; border-radius: 12px; text-align: center; color: white;">
                <div style="font-size: 18px; opacity: 0.9; margin-bottom: 15px;">현재 상태</div>
                <div style="font-size: 64px; font-weight: bold; margin: 20px 0;">${isHigh ? '높음' : '양호'}</div>
                <div style="font-size: 16px; opacity: 0.9;">${isHigh ? '⚠️ 인터럽트가 많습니다' : '✅ 안정적입니다'}</div>
            </div>
            <div style="margin-top: 20px; display: grid; gap: 10px;">
                <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; text-align: center;">
                    <div style="font-size: 13px; color: #666;">총 인터럽트</div>
                    <div style="font-size: 24px; font-weight: bold; color: #dc3545; margin-top: 5px;">${interrupts.length}건</div>
                </div>
                <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; text-align: center;">
                    <div style="font-size: 13px; color: #666;">총 손실 시간</div>
                    <div style="font-size: 24px; font-weight: bold; color: #ffc107; margin-top: 5px;">${totalHours.toFixed(1)}시간</div>
                </div>
            </div>
        </div></div>`;
...
async function loadProductivityDashboard(container) {
    ...
    try {
        const [projects, interrupts] = await Promise.all([getProjects(), getInterrupts()]);
        ...
        const totalInterruptHours = interrupts.reduce((sum, item) => sum + (parseFloat(item['예상소요시간']) || 0), 0);
        const avgInterruptTime = interrupts.length > 0 ? (totalInterruptHours / interrupts.length).toFixed(1) : 0;
        ...
        if (productivityScore >= 80) {
            grade = 'S (우수)'; gradeColor = '#28a745';
        } else if (productivityScore >= 60) {
            grade = 'A (양호)'; gradeColor = '#17a2b8';
        } else if (productivityScore >= 40) {
            grade = 'B (보통)'; gradeColor = '#ffc107';
        } else {
            grade = 'C (개선필요)'; gradeColor = '#dc3545';
        }
        
        let html = `
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div style="background: linear-gradient(135deg, ${gradeColor}, #764ba2); color: white; padding: 30px; border-radius: 12px; text-align: center;">
                ...
                <div style="display: grid; gap: 15px;">
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 8px;">
                        ...
                        <div style="font-size: 28px; font-weight: bold; color: var(--primary);">${completionRate}%</div>
                        <div style="font-size: 13px; color: #666; margin-top: 5px;">${completedProjects}개 / ${totalProjects}개 완료</div>
                    </div>
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 8px;">
                        <div style="font-size: 14px; color: #666; margin-bottom: 5px;">평균 인터럽트 시간</div>
                        <div style="font-size: 28px; font-weight: bold; color: #ffc107;">${avgInterruptTime}h</div>
                        <div style="font-size: 13px; color: #666; margin-top: 5px;">건당 평균 소요 시간</div>
                    </div>
                </div>
            </div>
        `;
